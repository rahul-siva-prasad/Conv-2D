...
CONV_2D_DATA* conv2D(int* image_arr, int image_size, int* kernel, int kernel_size,int stride,int bias);
...
CONV_2D_DATA* conv2D(int* image_arr, int image_size, int* kernel, int kernel_size,int stride,int bias) {
    int output_size = OUTPUT_KERNEL_SIZE(image_size,kernel_size,0,stride);
    ...
    for(int row_image=0;row_image<output_size+stride-1;row_image+=stride){
        for(int col_image=0;col_image<output_size+stride-1;col_image+=stride){
            ...
            for(int row_kernel = 0;row_kernel < kernel_size;row_kernel++){
                ...
                for(int col_kernel = 0;col_kernel < kernel_size;col_kernel++){
                    ...
                    int image_cell_val  = image_arr [row_start  * image_size  + col_start];
...
int main() {
    ...
    int bias=0;//5
    int stride =1;//1
    CONV_2D_DATA*ptr = conv2D((int*)image_arr, image_size, (int*)kernel, kernel_size,stride,bias);
